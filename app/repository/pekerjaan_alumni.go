...
func GetAllPekerjaan(ctx context.Context) ([]model.Pekerjaan, error) {
    rows, err := database.DB.QueryContext(ctx, `
        ...
        FROM pekerjaan ORDER BY created_at DESC`)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
func CreatePekerjaan(ctx context.Context, p *model.Pekerjaan) error {
    return database.DB.QueryRowContext(ctx, `
        INSERT INTO pekerjaan (alumni_id, nama_perusahaan, posisi_jabatan, bidang_industri, lokasi_kerja, gaji_range, tanggal_mulai_kerja, tanggal_selesai_kerja, status_pekerjaan, deskripsi_pekerjaan, created_at, updated_at)
...
func UpdatePekerjaan(ctx context.Context, p *model.Pekerjaan) (int64, error) {
    result, err := database.DB.ExecContext(ctx, `
        UPDATE pekerjaan SET nama_perusahaan=$1, posisi_jabatan=$2, bidang_industri=$3, lokasi_kerja=$4, gaji_range=$5, tanggal_mulai_kerja=$6, tanggal_selesai_kerja=$7, status_pekerjaan=$8, deskripsi_pekerjaan=$9, updated_at=$10 WHERE id=$11`,
...
func DeletePekerjaan(ctx context.Context, id int) (int64, error) {
    result, err := database.DB.ExecContext(ctx, "DELETE FROM pekerjaan WHERE id=$1", id)
    if err != nil {
        return 0, err
    }
    return result.RowsAffected()
